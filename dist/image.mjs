...
class F {
   ...
   */
  render(a) {
    return !a.file || Object.keys(a.file).length === 0 ? this.toggleStatus(F.status.EMPTY) : this.toggleStatus(F.status.UPLOADING), this.nodes.wrapper;
  }
  /**
   ...
   */
  showPreloader(a) {
    this.nodes.imagePreloader.style.backgroundImage = `url(${a})`, this.toggleStatus(F.status.UPLOADING);
  }
  /**
   ...
   */
  hidePreloader() {
    this.nodes.imagePreloader.style.backgroundImage = "", this.toggleStatus(F.status.EMPTY);
  }
  /**
   * Shows an image
   *
  ...
  fillImage(a) {
    ...
    let r = "load";
    i === "VIDEO" && (c.autoplay = !0, c.loop = !0, c.muted = !0, c.playsinline = !0, r = "loadeddata"), this.nodes.imageEl = j(i, this.CSS.imageEl, c), this.nodes.imageEl.addEventListener(r, () => {
      this.toggleStatus(F.status.FILLED), this.nodes.imagePreloader && (this.nodes.imagePreloader.style.backgroundImage = "");
  ...
  toggleStatus(a) {
    for (const i in F.status)
      Object.prototype.hasOwnProperty.call(F.status, i) && this.nodes.wrapper.classList.toggle(`${this.CSS.wrapper}--${F.status[i]}`, a === F.status[i]);
  }
  /**
    ...
    return function(i) {
      ...
      var r = function(o) {
        ...
        return d = d || null, b = b || "&", u = u || null, o ? function(p) {
          ...
          var f, y, w = p;
          if (u && (w = u + "[" + w + "]"), typeof o[p] == "object" && o[p] !== null)
            f = n(o[p], null, b, w);
          else {
...
class U {
  ...
  uploadSelectedFile({ onPreview: a }) {
    ...
    let c;
    this.config.uploader && typeof this.config.uploader.uploadByFile == "function" ? c = M.selectFiles({ accept: this.config.types }).then((r) => {
      i(r[0]);
  ...
  uploadByUrl(a) {
    let i;
    this.config.uploader && typeof this.config.uploader.uploadByUrl == "function" ? (i = this.config.uploader.uploadByUrl(a), _(i) || console.warn("Custom uploader method uploadByUrl should return a Promise")) : i = M.post({
